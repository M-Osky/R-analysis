...
#LINEAR DISCRIMINANT ANALYSIS is somehow something in petween the PLS and a multivariant analysis, it is in fact a regression analysis but it looks for the combination of variables that best explains the variance.
...
cat(maxcol, " variables: ", file="podarcisLDAs_out.txt", sep="", append=TRUE)
...
for (i in colgroup) {
	...
	if (numgroups==2)	{
		...
		print(ggplot(dataset) + geom_point(aes(lda.LD1, lda.LD2, colour = groups, fill=groups, shape = groups), size = 2.5) + 
