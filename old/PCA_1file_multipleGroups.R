...
getwd()
setwd("D:/Dropbox/MOSKY/CURRO/PODARCIS/Measurements/F1crosses/2017/Petra/PCA")
getwd()
...
#############################################
# Run this if the above commands fail for pcaMethods
...
#input file
measurespod = read.table("F1_2017_6th_allmeasures.csv", sep = ',', header =TRUE)	#read the data into a table, read headers
...
##########################################################################################################################################
########																															######
########			THIS IS ONLY FOR ANALYZING SUBSETS OF THE MAIN DATASET NEEDS TO BE RUN MANUALLY AND ONLY IF NEEDED				######
########					It will not work if your dataset is different than mine, so just igonore ir								######
########																															######
##########################################################################################################################################
##########################################################################################################################################

Mal <- measurespod[which (measurespod$Sex =="mal"),]		 #Extract only samples with "mal" on the Sex variable
str(Mal)
subsetMal <- Mal[c(1, 3:5, 7:15)]		 #Take everything but Sex and Group (as they included males and females and now they don't)
subsetMal
measurespod = subsetMal
str(measurespod)

# OR #

Fem <- measurespod[which (measurespod$Sex =="fem"),]		 #Extract only samples with "fem" on the Sex variable
str(Fem)
subsetFem <- Fem[c(1, 3:5, 7:15)]		 #Take everything but Sex and Group (as they included males and females and now they don't)
subsetFem
measurespod = subsetFem
str(measurespod)

# OR #

subseted <- measurespod[c(1:4, 6:11)]
str(subseted)
...
#############################	Run first the below chunk of code before proceeding with the rest
#########################################################################################################
...
varinfo <- scale(measurespod[c(firstvar:maxcol)])
head(varinfo)
...
#Is the information printed true?
...
##################################################################################################################################################
############################		IF EVERYTHING WAS ALRIGHT RUN THE REST OF THE SCRIPT AT ONCE		##########################################
##################################################################################################################################################
...
for (i in colgroup) {
	...
	if (numgroups<3)	{
		...
		myshapes=c(22, 21, 23:25, 15:18)
	} else if (numgroups==3)	{
		palette (c("red1", "goldenrod1", "purple3", "black", "green3", "orange", "magenta", "blue", "gray"))
		mypalette <- c("red1", "goldenrod1", "purple3", "black", "green3", "orange", "magenta", "blue", "gray")
		myshapes=c(22, 21, 25:23, 15:18)
	} else if (numgroups==4) {	
		...
		palette (c("red1", "goldenrod1", "purple3", "black", "green3", "orange", "magenta", "blue", "darkorange3", "darkgreen", "chartreuse3", "gray"))
		mypalette <- c("red1", "goldenrod1", "purple3", "black", "green3", "orange", "magenta", "blue", "darkorange3", "darkgreen", "chartreuse3", "gray")
		myshapes=c(22, 21, 25:23, 15:18)
	}
	...
	autoplot(phenopodPCA, data=podinfo) +
