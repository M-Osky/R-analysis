...
setwd("D:/Dropbox/MOSKY/CURRO/PODARCIS/Measurements/F1crosses/Analysis/analova")
...
measurespod = read.table("2017f1_length2.csv", sep = ',', header =TRUE)	#read the data into a table, read headers
str(measurespod)
...
for (i in colgroup) {
	...
	for (j in colvariab) {
		...
		print(ggboxplot (measurespod, x=coltags[i], y=coltags[j], color=coltags[i], legend="none", size=1, outlier.size=3.5, add="dotplot", add.params = list(color = coltags[i], binwidth=0.0008, alpha=0.5)) +
		...
		png(filename=nameplot, units="in", width=5, height=5, res=300)						#Save to file
		print(ggboxplot (tempdata, x=coltags[i], y=coltags[j], size=1, add="dotplot", add.params = list(color = coltags[i], binwidth=0.0008, alpha=0.5), color=coltags[i], legend="none", outlier.size=3.5) +
		geom_hline (yintercept=mean (tvariable), linetype = 2)+
		stat_summary(fun.y=mean, geom="point", shape=95, size=6) +
...
ggboxplot (tempdata, x=coltags[i], y=coltags[j], size=1, add="dotplot", add.params = list(color = coltags[i], binwidth=0.03, alpha=0.5), color=coltags[i], legend="none", outlier.size=3.5) +
geom_hline (yintercept=mean (tvariable), linetype = 2) +
stat_summary(fun.y=mean, geom="point", shape=95, size=6) +
geom_signif(comparisons = list(c(groupnames[1], groupnames[3])), y_position=5.2, test="wilcox.test", map_signif_level=TRUE, tip_length= 0.02) +
geom_signif(comparisons = list(c(groupnames[2], groupnames[4])), y_position=5.05, test="wilcox.test", map_signif_level=TRUE, tip_length= 0.02) +
geom_signif(comparisons = list(c(groupnames[1], groupnames[2]), c(groupnames[3], groupnames[4])), y_position=3.75, test="wilcox.test", map_signif_level=TRUE, tip_length=0, vjust=2) +
geom_signif(comparisons = list(c(groupnames[1], groupnames[4])), y_position=3.6, test="wilcox.test", map_signif_level=TRUE, tip_length= -0.02, vjust=3)+
scale_fill_manual(values=mypalette) + scale_colour_manual(values=mypalette)
