...
myGtest1 <- global.rtest(obj1$tab,mySpca1$lw,nperm=9999)
myGtest1
plot(myGtest1)

#vemos lo mismo para la estructura intra (local)

myLtest1 <- local.rtest(obj1$tab,mySpca1$lw,nperm=9999)
myLtest1
plot(myLtest1)
...
annot <- function(){
title("sPCA - Type 1, 1 glob score")
points(x,y)
}
...
#Provamos con más eigenvalues. Primero 2, 3 positivos, 2 y 1 y luego 3 y 1


#2positivos


mySpca2 <- spca(obj1,type=1,scannf=FALSE,plot.nb=TRUE, nfposi=2,nfnega=0)
...
myGtest2 <- global.rtest(obj1$tab,mySpca2$lw,nperm=9999)
myGtest2
plot(myGtest2)

#vemos lo mismo para la estructura intra (local)

myLtest2 <- local.rtest(obj1$tab,mySpca2$lw,nperm=9999)
myLtest2
plot(myLtest2)
...
#Intentamos verlo mejor
colorplot(mySpca2,cex=3,main="colorplot of mySpca, first 2 global scores")
...
annot <- function(){
  title("sPCA - Type 1, 2 glob scores")
  points(x,y)
}
...
myGtest3<- global.rtest(obj1$tab,mySpca3$lw,nperm=9999)
myGtest3
plot(myGtest3)

#vemos lo mismo para la estructura intra (local)

myLtest3<- local.rtest(obj1$tab,mySpca3$lw,nperm=9999)
myLtest3
plot(myLtest3)
...
#Intentamos verlo mejor
colorplot(mySpca3, cex=3,main="colorplot of mySpca, first 2 global score + 1 local")
...
mySpca4 <- spca(obj1,type=1,scannf=FALSE,plot.nb=TRUE, nfposi=3,nfnega=0)

#vemos
mySpca4
#representamos los valores
barplot(mySpca4$eig, main="A variant of the plotnn of sPCA eigenvalues", 
        col=spectral(length(mySpca4$eig)))
legend("topright", fill=spectral(2),
       leg=c("Global structures", "Local structures"))
abline(h=0,col="grey")
...
myGtest4<- global.rtest(obj1$tab,mySpca4$lw,nperm=9999)
myGtest4
plot(myGtest4)

#vemos lo mismo para la estructura intra (local)

myLtest4<- local.rtest(obj1$tab,mySpca4$lw,nperm=9999)
myLtest4
plot(myLtest4)
...
#Intentamos verlo mejor
colorplot(mySpca4, cex=3,main="colorplot of mySpca, first 3 global score")

#Podemos intertar mejorarlo
x <- other(obj1)$xy[,1]
y <- other(obj1)$xy[,2]
temp6 <- interp(x, y, mySpca4$li[,1])
image(temp6, col=azur(100))
points(x,y)
...
interpX <- seq(min(x),max(x),le=200)
interpY <- seq(min(y),max(y),le=200)
temp7 <- interp(x, y, mySpca4$ls[,1], xo=interpX, yo=interpY)
image(temp7, col=azur(100))
points(x,y)
...
mySpca5 <- spca(obj1,type=1,scannf=FALSE,plot.nb=TRUE, nfposi=3,nfnega=1)

#vemos
mySpca5
#representamos los valores
barplot(mySpca5$eig, main="A variant of the plotnn of sPCA eigenvalues", 
        col=spectral(length(mySpca5$eig)))
legend("topright", fill=spectral(2),
       leg=c("Global structures", "Local structures"))
abline(h=0,col="grey")
...
myGtest5<- global.rtest(obj1$tab,mySpca5$lw,nperm=9999)
myGtest5
plot(myGtest5)

#vemos lo mismo para la estructura intra (local)

myLtest5<- local.rtest(obj1$tab,mySpca5$lw,nperm=9999)
myLtest5
plot(myLtest5)
...
#Intentamos verlo mejor
colorplot(mySpca5, cex=3,main="colorplot of mySpca, first 3 global + 1 local score")

#Podemos intertar mejorarlo
x <- other(obj1)$xy[,1]
y <- other(obj1)$xy[,2]
temp6 <- interp(x, y, mySpca5$li[,1])
image(temp6, col=azur(100))
points(x,y)
...
interpX <- seq(min(x),max(x),le=200)
interpY <- seq(min(y),max(y),le=200)
temp7 <- interp(x, y, mySpca5$ls[,1], xo=interpX, yo=interpY)
image(temp7, col=azur(100))
points(x,y)
...
barplot(mySpca51$eig, main="Plot of sPCA eigenvalues", 
...
mySpca5 <- spca(obj1,type=5,d1=0,d2=15380,scannf=FALSE, plot.nb=TRUE, nfposi=3,nfnega=3)
mySpca5
...
#Lo modificamos un poco, usando valores más exagerados para verlo más claro
interpX <- seq((min(x)-1000),(max(x)+1000),le=200)
interpY <- seq((min(y)-1000),(max(y)+1000),le=200)
temp <- interp(x, y, mySpca5$ls[,1], xo=interpX, yo=interpY)
image(temp, col=azur(100))
points(x,y)
...
myPal <- colorRampPalette(c("firebrick2", "white", "lightslateblue"))
annot <- function(){
  title("sPCA (Type 5) - Two global scores")
  points(x,y)
}
filled.contour(temp, color.pal=myPal, nlev=50,
               key.title=title("Lagged \nscore"), plot.title=annot())
...
mySpca5 <- spca(obj1,type=5,d1=0,d2=15380,scannf=FALSE, plot.nb=TRUE, nfposi=1,nfnega=0)
mySpca5
...
myPal <- colorRampPalette(c("firebrick2", "white", "lightslateblue"))
annot <- function(){
  title("sPCA - Type 5, 1 global score")
  points(x,y)
}
filled.contour(temp, color.pal=myPal, nlev=50,
               key.title=title("lagged \nscore 1"), plot.title=annot())
...
#Otro acercamiento segun el tutorial
mySpca5 <- spca(obj1,type=5,d1=0,d2=15380,scannf=FALSE, plot.nb=TRUE, nfposi=2,nfnega=0)
mySpca5

#Genind13.graph <- chooseCN(Genind13$other$xy,type=5,d1=0,d2=15650, plot=FALSE, res="listw")
#Genind13.graph
#plot(Genind13.graph, Genind13$other$xy)

#Moran Test
#pc1.mctest <- moran.mc(mySpca5$li[,1], Genind13.graph, 9999)
#plot(pc1.mctest)
#pc1.mctest
